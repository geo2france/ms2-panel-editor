            ...
            action$.ofType(LOAD_FEATURE_INFO, ERROR_FEATURE_INFO)
                .mergeMap((loadFeatInfoAction) => {
                    ...
                    const noResultFeatures = loadFeatInfoAction.type === LOAD_FEATURE_INFO
                        && (
                            Array.isArray(loadFeatInfoAction?.layerMetadata?.features)
                                ? loadFeatInfoAction.layerMetadata.features.length === 0
                                : Array.isArray(loadFeatInfoAction?.data?.features)
                                    ? loadFeatInfoAction.data.features.length === 0
                                    : typeof loadFeatInfoAction?.data === 'string'
                                        && loadFeatInfoAction.data.includes("no features were found")
                        );
