...
describe('identify Epics', () => {
    ...
    it('test zoomToVisibleAreaEpic ignores non-string feature info payloads', (done) => {
        registerHook('RESOLUTION_HOOK', undefined);
        ...
        const sentActions = [
            featureInfoClick({ latlng: { lat: 36.95, lng: -79.84 } }),
            loadFeatureInfo(1, { features: [] })
        ];

        testEpic(zoomToVisibleAreaEpic, 1, sentActions, (actions) => {
            try {
                expect(actions.length).toBe(1);
